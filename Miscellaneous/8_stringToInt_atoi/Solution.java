...
class Solution
{
	...
	public int myAtoi(String str)
	{
		...
		if(str == null || str.length() == 0) return 0;  ////check if str is null
		...
		if(str.charAt(index) == '+' || str.charAt(index) == '-')  ////check for sign 
		{
			sign = str.charAt(index) == '+'? 1 : -1;
			index++;
		}

		while(index < str.length())   // convert to the actual number and make sure it's not overflow
		{
			...
			if(Integer.MAX_VALUE < total * 10 || (Integer.MAX_VALUE == total * 10 && Integer.MAX_VALUE % 10 < digit))
